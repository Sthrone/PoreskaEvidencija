...
@Component({
  selector: 'app-dodaj-kupca',
  templateUrl: './dodaj-kupca.component.html',
  styleUrls: ['./dodaj-kupca.component.scss'],
  animations: [routerTransition()]
})
export class DodajKupcaComponent implements OnInit 
{
  p: Partner;
  ...
  NapraviKupca()
  {
    ...
    if (p1 && p2 && p3 && p4)
    {
      let tekst = this.translate.instant('_62');
      ...
      this.dijalogRef.afterClosed().subscribe((res) => 
      {
        if (res == true)
        {
          ...
          this.p.id_uloge = 1;    // Kupac

          this.partnerInfo.DodajPartnera(this.p);
        }
        else
          this.toastr.info(this.translate.instant('_63'));
      });
    }
